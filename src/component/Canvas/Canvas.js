...
class Canvas extends React.Component {
  ...
  draw = (ctx) => {
    ...
    ctx.fillRect(0, 0, this.props.canvas.canvasWidth, this.props.canvas.canvasHeight);
    ...
    for(let i = 0; i < drops.length; i++) {
      ...
      if(drops[i]*FONT_SIZE > this.props.canvas.canvasHeight && Math.random() > 0.975)
        drops[i] = 0;
      drops[i]++;
    }
  };
  ...
  onChangeColumns = () => {
    this.props.changeSize(window.innerWidth, document.body.scrollHeight, window.innerWidth/FONT_SIZE);
    for(let x = 0; x < this.props.canvas.columns; x++) drops[x] = 1;
  };
